...
export const Scene11DiscoverZero: React.FC<Scene11Props> = ({ onComplete }) => {
  // Starts with 3 dates on the plate.
  // Child taps each date to make it disappear: 3 -> 2 -> 1 -> empty!
  // Brief pause -> giant Hero ٠ reveals!
  ...
  const handleDateClick = () => {
    ...
    if (nextRemaining === 0) {
      setTimeout(() => {
        ...
        sound.speakArabic('ما بقي شيء! صِفْر');
        confetti({
          particleCount: 65,
          spread: 85,
          origin: { y: 0.5 },
  ...
  return (
    <div className="relative w-full h-full flex flex-col justify-between overflow-hidden">
      ...
      <InstructionBar
        instruction={datesRemaining > 0 ? 'المس التمر!' : 'ما بقي شيء!'}
      />
      ...
      <div className="relative flex-1 flex flex-col items-center justify-center gap-6 z-10 px-16">
        ...
        {zeroRevealed && (
          <div className="flex flex-col items-center animate-bounce z-20">
            <HeroNumeral number={0} label="صِفْر" size="hero" />
          </div>
        )}
      </div>
    </div>
